...
// Skype OAuth2 Sign-in Parameters
const clientId = 'c3c093c3-cf82-4e81-82fc-6317d208d9fb';
const tenantId = 'a50dae87-f534-4bb5-ab29-38087da4ecd4';
const redirectUri = 'http://localhost:9966/';  // Change this to your actual redirect URL
const clientSecret = '26858c5a-6c6b-4b01-b47a-fcb17165eb82';

// Ensure the DOM is fully loaded before executing script
document.addEventListener('DOMContentLoaded', function () {
  const skypeLoginBtn = document.getElementById('skypeLoginBtn');

  if (skypeLoginBtn) {
    // Add event listener to initiate OAuth flow on button click
    skypeLoginBtn.addEventListener('click', function () {
      const oauthUrl = `https://login.microsoftonline.com/${tenantId}/oauth2/v2.0/authorize?client_id=${clientId}&response_type=code&redirect_uri=${encodeURIComponent(redirectUri)}&response_mode=query&scope=OnlineMeetings.ReadWrite%20User.Read`;

      // Redirect to Microsoft OAuth2 authorization URL
      window.location.href = oauthUrl;
    });
  } else {
    console.error("Button with ID 'skypeLoginBtn' not found.");
  }

  // Check if we are redirected back with an authorization code
    ...
    fetch(`https://login.microsoftonline.com/${tenantId}/oauth2/v2.0/token`, {
      ...
      .then(data => {
        if (data.access_token) {
          ...
          // Use the access token to create a Skype meeting
          setupSkypeMeeting(accessToken);
        } else {
...
// Function to create a Skype meeting using the access token
function setupSkypeMeeting(token) {
  fetch('https://graph.microsoft.com/v1.0/me/onlineMeetings', {
    ...
    body: JSON.stringify({
      ...
      subject: 'Sign Language Translator Meeting',
    }),
  })
    .then(response => response.json())
    .then(meetingData => {
      if (meetingData.joinUrl) {
        ...
        // Open the Skype meeting in a new window
        window.open(joinUrl, '_blank');
      } else {
        console.error('Error creating Skype meeting:', meetingData);
      }
    })
    .catch(error => console.error('Error creating Skype meeting:', error));
}
...
// Now you can use this token to make calls to Skype API
...
class Main {
  constructor() {
    ...
    // Call getCredentials and then setup the video call
    // this.getCredentials = this.getCredentials.bind(this);

    // // Call getCredentials and then setup the video call
    // this.getCredentials().then(() => {
    //   this.setupTokBoxVideoCall();
    // });
  ...
  // setupTokBoxVideoCall() {
  //   const session = OT.initSession(this.apiKey, this.sessionId);

  //   // Subscribe to a newly created stream from another user
  //   session.on('streamCreated', (event) => {
  //     session.subscribe(event.stream, 'videoCall', {
  //       insertMode: 'append',
  //       width: '100%',
  //       height: '100%'
  //     });
  ...
  //     if (error) {
  //       console.error('Failed to connect to the session:', error.message);
  //     } else {
  //       // Start publishing the local webcam feed
  //       const publisher = OT.initPublisher('videoHolder', {
  //         insertMode: 'append',
  //         width: '100%',
  //         height: '100%'
  //       });
  ...
  createTranslateBtn() {
    ...
    this.predButton.addEventListener('click', () => {
      ...
      const exampleCount = this.knn.getClassExampleCount();
      // check if training is complete   //here changed code check properly
      if (Math.max(...exampleCount) > 0) {

        // Ensure elements are initialized and accessible
        if (this.video) this.video.style.display = "inline-block";
        if (this.localVideo) this.localVideo.style.display = "inline-block";
        if (this.remoteVideo) this.remoteVideo.style.display = "inline-block"; // turn on video from webcam in case it's off

        if (this.videoCall) this.videoCall.style.display = "none"; // turn off video call in case it's on
        if (this.videoCallBtn) this.videoCallBtn.style.display = "block";

        if (this.backToTrainButton) this.backToTrainButton.style.display = "block";
        ...
        if (this.stageTitle) this.stageTitle.innerText = "Translate";
        if (this.stageInstruction) this.stageInstruction.innerText = "Start Translating with your Start Gesture.";

        // Remove training UI
        if (this.trainingContainer) this.trainingContainer.style.display = "none";
        if (this.trainedCardsHolder) this.trainedCardsHolder.style.marginTop = "130px";

        // Display translation holder that contains translated text
        if (this.translationHolder) this.translationHolder.style.display = "block";

        if (this.predButton) this.predButton.style.display = "none";
  ...
  /*This function displays the button that start video call.*/ //here too changed code check properly
  createVideoCallBtn() {
    ...
    this.videoCallBtn.addEventListener('click', () => {
      if (this.stageTitle) this.stageTitle.innerText = "Video Call";
      if (this.stageInstruction) this.stageInstruction.innerText = "Translate Gestures to talk to people on Video Call";
      ...
      if (this.videoCall) this.videoCall.style.display = "block";
      if (this.videoCallBtn) this.videoCallBtn.style.display = "none";
      if (this.backToTrainButton) this.backToTrainButton.style.display = "none";
...
class PredictionOutput {
  constructor() {
    ...
    this.apiKey = 'AIzaSyAOVtpWogd81-8t-2Nu5PdUNWkfj-73fEY'; // Replace with your API key
  ...
  textOutput(word, gestureCard, gestureAccuracy) {
    ...
    if (word == "start") {
      ...
      // Translate the text if it's not "start" or "stop"
      this.translateText(this.translationText.innerText, selectedLang)
        .then(translatedText => {
          this.translationText.innerText = translatedText;
